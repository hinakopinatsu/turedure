...
type Exchange = { id: number|string; person: string; visibility: "public" | "private"; waiting: boolean; participant?:boolean; poems: { id?:string; senderUserId?:string; authorNumber?:string; fromMe: boolean; lines: string[]; scentKey?:ScentKey|null; branchKey?:BranchKey|null }[] };
...
const scents:Record<ScentKey,{name:string;note:string;arrival:string}>={baika:{name:"梅花",note:"胸の奥が、ふと明るむような気配",arrival:"梅花の香、ほのかに残れり"},kayo:{name:"荷葉",note:"心ほどける、涼やかな気配",arrival:"荷葉の香、涼やかに残れり"},kikka:{name:"菊花",note:"去りゆくものを惜しむ、静かな余韻",arrival:"菊花の香、静かに残れり"},kurobo:{name:"黒方",note:"胸の底に秘めた、深い情",arrival:"黒方の香、深く残れり"}};
const branches:Record<BranchKey,{name:string;note:string;season:Season}>={ume:{name:"梅",note:"凛と立つ、一筋の想い",season:"spring"},yamabuki:{name:"山吹",note:"明るく、やわらかな気配",season:"spring"},fuji:{name:"藤",note:"あなたへ垂れる、やわらかな想い",season:"spring"},tachibana:{name:"橘",note:"清らかで、懐かしい気配",season:"summer"},unohana:{name:"卯の花",note:"白く、かすかな気配",season:"summer"},hasu:{name:"蓮",note:"水辺にひらく静かな気配",season:"summer"},hagi:{name:"萩",note:"秋風にほどける気配",season:"autumn"},susuki:{name:"薄",note:"風に揺れる、定まらぬ想い",season:"autumn"},kiku:{name:"菊",note:"澄みわたる静かな気配",season:"autumn"},matsu:{name:"松",note:"変わらず、そこに在る想い",season:"winter"},ume_winter:{name:"寒梅",note:"寒気に咲く凛とした気配",season:"winter"},snow_branch:{name:"雪折れの枝",note:"雪の重みを残す気配",season:"winter"}};
const featuredBranches:BranchKey[]=["ume","fuji","susuki","matsu"];
...
const mapExchange=(exchange:any,userId:string):Exchange=>{
  ...
  const root=exchange.root?[{fromMe:exchange.root.user_id===userId,authorNumber:String(exchange.root.users?.user_number??"—"),lines:linesFromRow(exchange.root)}]:[];
  const replies=[...(exchange.exchange_poems??[])].sort((a:any,b:any)=>a.created_at.localeCompare(b.created_at)).map((poem:any)=>({id:poem.id,senderUserId:poem.sender_user_id,authorNumber:String(poem.sender?.user_number??"—"),fromMe:poem.sender_user_id===userId,lines:linesFromRow(poem),scentKey:poem.scent_key as ScentKey|null,branchKey:poem.branch_key as BranchKey|null}));
...
function GiftComposer({ mode, onClose, onSend, onDelivered }: { mode: GiftMode; onClose: () => void; onSend: (lines: string[], visibility: "public" | "private", scentKey:ScentKey|null, branchKey:BranchKey|null) => Promise<void>;onDelivered:()=>void }) {
  ...
  return <main className={`gift-compose page-in ${entrusted?"entrusted":""}`}>{entrusted&&<div className="courier-moment" role="status"><i aria-hidden="true"/><p>文は、使いの手へ渡りました</p></div>}<button className="close" disabled={submitting||entrusted} onClick={onClose}>そっと閉じる</button><SeasonMark /><header><p>{mode.recipient}番へ</p><h1>{mode.kind === "reply" ? "返歌を詠む" : "文をしたためる"}</h1></header>{mode.poem && <div className="source-poem"><p>心を寄せた歌</p><PoemText lines={mode.poem.lines} vertical /></div>}{mode.kind === "reply" && <fieldset className="visibility"><legend>この歌への返歌を</legend><label><input type="radio" checked={visibility === "public"} onChange={() => {setVisibility("public");setScentKey(null);setBranchKey(null)}} /><span>歌会に残す<small>皆が読める贈答歌として</small></span></label><label><input type="radio" checked={visibility === "private"} onChange={() => setVisibility("private")} /><span>文として届ける<small>この方だけに、そっと</small></span></label></fieldset>}<div className="line-inputs">{lines.map((line,i) => <label key={i}><span>{guides[i]}</span><input value={line} onChange={e => setLines(old => old.map((v,x) => x === i ? e.target.value : v))} /><em>{line.length || ""}</em></label>)}</div>{visibility==="private"&&<details className="adornment-disclosure"><summary>趣を添える<span>{scentKey||branchKey?`${scentKey?scents[scentKey].name:"香なし"}・${branchKey?branches[branchKey].name:"枝なし"}`:"香と一枝は任意です"}</span></summary><div className="letter-adornments"><fieldset className="scent-choice"><legend>香を添える<small>一首に、感情の温度をまとわせる</small></legend><div className="adornment-options scent-options"><button type="button" className={scentKey===null?"chosen":""} onClick={()=>setScentKey(null)}><b>香なし</b></button>{(Object.keys(scents) as ScentKey[]).map(key=><button type="button" className={scentKey===key?"chosen":""} aria-pressed={scentKey===key} onClick={()=>setScentKey(key)} key={key}><i className={`scent-swatch scent-${key}`} aria-hidden="true"/><b>{scents[key].name}</b><small>{scents[key].note}</small></button>)}</div></fieldset><fieldset className="branch-choice"><legend>一枝、添えますか<small>一首に、想いの姿を添える</small></legend><div className="adornment-options branch-options"><button type="button" className={branchKey===null?"chosen":""} onClick={()=>setBranchKey(null)}><b>添えない</b></button>{featuredBranches.map(key=><button type="button" className={branchKey===key?"chosen":""} aria-pressed={branchKey===key} onClick={()=>setBranchKey(key)} key={key}><i className={`branch-line branch-${key}`} aria-hidden="true"/><b>{branches[key].name}</b><small>{branches[key].note}</small></button>)}</div></fieldset></div></details>}<button className="primary" disabled={submitting||entrusted||!lines.every(Boolean)} aria-busy={submitting} onClick={send}>{submitting?"一首を届けています":"一首を結ぶ"}</button><p className="waiting-rule">この一首が返されるまで、<br />次の文は送れません。</p></main>;
}

function LetterTrace({poem,reveal=false}:{poem:Exchange["poems"][number];reveal?:boolean}) {
  if(!poem.scentKey&&!poem.branchKey)return null;
  const branch=poem.branchKey?branchDetails(poem.branchKey):null;
  return <div className={`letter-presence ${reveal?"is-arriving":""} ${poem.scentKey?`scent-${poem.scentKey}`:""}`}><i className="scent-aura" aria-hidden="true"/>{branch&&<i className={`branch-figure branch-${poem.branchKey}`} aria-hidden="true"/>}<div className="letter-trace">{poem.scentKey&&<p><span>香　{scents[poem.scentKey].name}</span><small>{scents[poem.scentKey].note}</small></p>}{branch&&<p><span>{branch.name}一枝を添えて</span><small>{branch.note}</small></p>}</div></div>;
}
...
function PublicReplies({ exchanges, onClose }: { exchanges: Exchange[]; onClose: () => void }) {
  ...
  return <main className="exchange-page public-replies-page page-in"><button className="close" onClick={onClose}>歌会へ戻る</button><header><p>歌会に残りし言の葉</p><h1>返歌 {replies.length}首</h1></header>{root && <section className="reply-root"><span>元歌</span><PoemText lines={root.lines} vertical /></section>}<div className="exchange-flow">{replies.map((reply, index) => <article key={`${reply.person}-${index}`} className={reply.fromMe ? "from-me" : "from-them"}><span>返歌 {index + 1}</span><PoemText lines={reply.lines} vertical /><small>— {reply.authorNumber??"—"}番</small></article>)}</div><p className="all-replies-note">寄せられた返歌を、すべて。</p></main>;
}
...
function AuthenticatedHome({initialNumber}:{initialNumber:number}) {
  ...
  const [revealPoemId,setRevealPoemId]=useState<string>();
  ...
  const openLetter=(exchange:Exchange)=>{setExchangeReturn("letters");setOpenExchange(exchange);setRevealPoemId(undefined);if(!dbUserId||viewedExchangePoemIds===null)return;const unseenPoems=exchange.poems.filter(poem=>!poem.fromMe&&poem.id&&!viewedIds.includes(poem.id));const adorned=[...unseenPoems].reverse().find(poem=>poem.scentKey||poem.branchKey);setRevealPoemId(adorned?.id);const unseen=unseenPoems.map(poem=>poem.id!);if(!unseen.length)return;void markExchangePoemsViewed(dbUserId,unseen).then(()=>setViewedExchangePoemIds(old=>Array.from(new Set([...(old??[]),...unseen])))).catch(error=>setDbError(error instanceof Error?error.message:"届きし文を確認済みにできませんでした"))};
  ...
  if (openExchange) return <ExchangeView exchange={openExchange} revealPoemId={revealPoemId} returnLabel={exchangeReturn === "letters" ? "文箱へ戻る" : "歌会へ戻る"} onClose={() => {setRevealPoemId(undefined);setOpenExchange(null);setView(exchangeReturn)}} onReply={() => setGift({kind:"letter",recipient:openExchange.person,exchangeId:openExchange.id,visibility:openExchange.visibility})} />;
