...
import { addExchangePoem, beginExchange as saveExchange, ensureUser, getAwarePoemIds, getExchanges, getMyPoemAwareCount, getMyPoems, getPendingResults, getPeekPoems, getPinnedPoemIds, getPoems, getPublicExchanges, getTodayMatches, getTodayTheme, getViewedExchangePoemIds, markExchangePoemsViewed, markResultsViewed, recordGlimpse, replacePinned, setAware, signIn, signOut, signUp, submitPoem, vote as saveVote, type DbTheme, type PendingResults } from "@/lib/supabase/repository";
...
type Exchange = { id: number|string; person: string; visibility: "public" | "private"; waiting: boolean; participant?:boolean; poems: { id?:string; senderUserId?:string; fromMe: boolean; lines: string[] }[] };
...
const mapExchange=(exchange:any,userId:string):Exchange=>{
  ...
  const replies=[...(exchange.exchange_poems??[])].sort((a:any,b:any)=>a.created_at.localeCompare(b.created_at)).map((poem:any)=>({id:poem.id,senderUserId:poem.sender_user_id,fromMe:poem.sender_user_id===userId,lines:linesFromRow(poem)}));
