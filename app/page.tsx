...
import { addExchangePoem, beginExchange as saveExchange, ensureUser, getAwarePoemIds, getExchanges, getMyPoemAwareCount, getMyPoems, getPendingResults, getPeekPoems, getPinnedPoemIds, getPoems, getPublicExchanges, getTodayMatches, getTodayTheme, markResultsViewed, recordGlimpse, replacePinned, setAware, signIn, signOut, signUp, submitPoem, vote as saveVote, type DbTheme, type PendingResults } from "@/lib/supabase/repository";
...
type Exchange = { id: number|string; person: string; visibility: "public" | "private"; waiting: boolean; participant?:boolean; poems: { fromMe: boolean; lines: string[] }[] };
...
const mapExchange=(exchange:any,userId:string):Exchange=>{
  ...
  const replies=[...(exchange.exchange_poems??[])].sort((a:any,b:any)=>a.created_at.localeCompare(b.created_at)).map((poem:any)=>({fromMe:poem.sender_user_id===userId,lines:linesFromRow(poem)}));
...
function Nav({ view, setView, unlocked }: { view: View; setView: (v: View) => void; unlocked: boolean }) {
  return <nav className="bottom-nav" aria-label="主要な頁">{nav.map((item) => {const awaseLocked=item.id==="utaawase"&&!unlocked;return <button key={item.id} className={view === item.id ? "active" : ""} disabled={awaseLocked} aria-label={awaseLocked?"歌合（本日の歌を詠んだ後に開きます）":item.label} onClick={() => {if(!awaseLocked)setView(item.id)}}><span>{item.label}</span>{((item.id === "gathering"||item.id==="utaawase") && !unlocked) && <i className="lock-dot" />}</button>})}</nav>;
}
...
function Gathering({ unlocked, poems, onPeek, onReply, onOpenReplies, publicExchanges, awareIds, onAware }: { unlocked: boolean; poems: Poem[]; onPeek: (poem: Poem) => void; onReply: (poem: Poem) => void; onOpenReplies: (exchanges: Exchange[]) => void; publicExchanges: Exchange[]; awareIds: number[]; onAware: (id: number) => void }) {
  ...
  return <main className="gathering page-in"><SeasonalScenery /><SeasonMark /><header className="section-head"><p>本日の歌会</p><h1>{theme?.title}</h1><span>{page + 1} / 2</span></header><div className="poem-list">{visible.map((p) => {
...
function AuthenticatedHome({initialNumber}:{initialNumber:number}) {
  ...
  const [liveMatches,setLiveMatches]=useState<{id?:string;left:Poem;right:Poem}[]>([]);
  ...
  if (composing) return <Composer title={todayTheme.title} onCancel={() => setComposing(false)} onSubmit={async(value) => {try{if(dbUserId&&themeId){await submitPoem(dbUserId,themeId,value);const [rows,myRows]=await Promise.all([getPoems(themeId),getMyPoems(dbUserId)]);setLivePoems(rows.map((row,index)=>({id:1000+index,dbId:row.id,userId:row.user_id,author:String(row.users?.user_number??"—"),lines:[row.line_1,row.line_2,row.line_3,row.line_4,row.line_5]})));setOwnPoems(myRows.map((row:any)=>({id:row.id,lines:[row.line_1,row.line_2,row.line_3,row.line_4,row.line_5],date:row.themes?.date??"",theme:row.themes?.title??""})));}setLines(value);setSubmitted(true);setComposing(false)}catch(error){setDbError(error instanceof Error?error.message:"歌を保存できませんでした")}}} />;
